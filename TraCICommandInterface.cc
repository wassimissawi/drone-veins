...
std::string TraCICommandInterface::DroneUAV::getRoadId()
{
    return traci->genericGetString(CMD_GET_DRONEUAV_VARIABLE, uavId, VAR_ROAD_ID, RESPONSE_GET_DRONEUAV_VARIABLE);
}

std::string TraCICommandInterface::DroneUAV::getLaneId()
{
    return traci->genericGetString(CMD_GET_DRONEUAV_VARIABLE, uavId, VAR_LANE_ID, RESPONSE_GET_DRONEUAV_VARIABLE);
}

int32_t TraCICommandInterface::DroneUAV::getLaneIndex()
{
    return traci->genericGetInt(CMD_GET_DRONEUAV_VARIABLE, uavId, VAR_LANE_INDEX, RESPONSE_GET_DRONEUAV_VARIABLE);
}

std::string TraCICommandInterface::DroneUAV::getTypeId()
{
    return traci->genericGetString(CMD_GET_DRONEUAV_VARIABLE, uavId, VAR_TYPE, RESPONSE_GET_DRONEUAV_VARIABLE);
}

double TraCICommandInterface::DroneUAV::getMaxSpeed()
{
    return traci->genericGetDouble(CMD_GET_DRONEUAV_VARIABLE, uavId, VAR_MAXSPEED, RESPONSE_GET_DRONEUAV_VARIABLE);
}

double TraCICommandInterface::DroneUAV::getLanePosition()
{
    return traci->genericGetDouble(CMD_GET_DRONEUAV_VARIABLE, uavId, VAR_LANEPOSITION, RESPONSE_GET_DRONEUAV_VARIABLE);
}

std::list<std::string> TraCICommandInterface::DroneUAV::getPlannedRoadIds()
{
    return traci->genericGetStringList(CMD_GET_DRONEUAV_VARIABLE, uavId, VAR_EDGES, RESPONSE_GET_DRONEUAV_VARIABLE);
}

std::string TraCICommandInterface::DroneUAV::getRouteId()
{
    return traci->genericGetString(CMD_GET_DRONEUAV_VARIABLE, uavId, VAR_ROUTE_ID, RESPONSE_GET_DRONEUAV_VARIABLE);
}
//end_drone
...
void TraCICommandInterface::DroneUAV::stopAt(std::string roadId, double pos, uint8_t laneid, double radius, simtime_t waittime)
{
    uint8_t variableId = CMD_STOP;
    uint8_t variableType = TYPE_COMPOUND;
    int32_t count = 4;
    ...
    TraCIBuffer buf = connection->query(CMD_SET_DRONEUAV_VARIABLE, TraCIBuffer() << variableId << uavId << variableType << count << edgeIdT << edgeId << stopPosT << stopPos << stopLaneT << stopLane << durationT << duration);
    ASSERT(buf.eof());
}
