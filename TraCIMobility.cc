...
//for drones
void TraCIMobility::nextPositionUAV(const Coord& position, std::string road_id, double speed, Heading heading, VehicleSignalSet signals)
{
    EV_DEBUG << "nextPosition " << position.x << " " << position.y << " " << road_id << " " << speed << " " << heading << std::endl;
    isPreInitialized = false;
    this->roadPosition = position;
...
void TraCIMobility::changePositionUAV()
{
    // ensure we're not called twice in one time step
    ASSERT(lastUpdate != simTime());
...
//for  drones
Coord TraCIMobility::calculateHostPositionUAV(const Coord& droneUAVPos) const
{
    ...
    if (hostPositionOffset >= 0.001) {
        ...
        corPos2 = droneUAVPos - (heading.toCoord() * hostPositionOffset);
    }
    else {
