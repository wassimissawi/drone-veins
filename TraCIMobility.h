...
public:
    ...
    TraCIMobility()
        ...
        , droneUAVCommandInterface(nullptr)
    {
    }
    ~TraCIMobility() override
    {
        delete vehicleCommandInterface;
        delete droneUAVCommandInterface;
    }
    ...
    //for drones
    virtual void nextPositionUAV(const Coord& position, std::string road_id = "", double speed = -1, Heading heading = Heading::nan, VehicleSignalSet signals = {VehicleSignal::undefined});
    virtual void changePositionUAV();
    //ends
   ...
   virtual std::string getExternalId() const
    ...
    //for drones
    virtual void setExternalIdUAV(std::string external_idUAV)
                {
                    this->external_idUAV = external_idUAV;
                }
    virtual std::string getExternalIdUAV() const
                {
                    if (external_idUAV == "") throw cRuntimeError("TraCIMobility::getExternalId called with no external_id set yet");
                    return external_idUAV;
                }
    ...
    //for drones
    virtual TraCICommandInterface::DroneUAV* getDroneUAVCommandInterface() const
          {
              if (!droneUAVCommandInterface) droneUAVCommandInterface = new TraCICommandInterface::DroneUAV(getCommandInterface()->droneUAV(getExternalId()));
              return droneUAVCommandInterface;
          }
...
protected:
    ...
    mutable TraCICommandInterface::DroneUAV* droneUAVCommandInterface;
    double last_speed;
    ...
    //for drones
    Coord calculateHostPositionUAV(const Coord& droneUAVPos) const;
