...
public:
    ...
    bool addDroneUAV(std::string DroneUAVId, std::string DroneUAVTypeId, std::string routeIdUAV, simtime_t emitTime_st = DEPART_TIME_TRIGGERED, double emitPosition = DEPART_POSITION_BASE, double emitSpeed = DEPART_SPEED_MAX, int8_t emitLane = DEPART_LANE_BEST);
    class DroneUAV {
        public:
        DroneUAV(TraCICommandInterface* traci, std::string uavId)
                : traci(traci)
                , uavId(uavId)
            {
                connection = &traci->connection;
    ...
    DroneUAV droneUAV(std::string uavId)
       {
           return DroneUAV(this, uavId);
       }
    // Road methods
    class Road {
    public:
    ...
    std::list<std::string> getDroneUavTypeIds();
